...
function setup_vendor() {
    ...
    if [ ! -d "$YODITA_ROOT" ]; then
        echo "\$YODITA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$YODITA_ROOT/$OUTDIR" ]; then
        mkdir -p "$YODITA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$YODITA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$YODITA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$YODITA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The Yodita Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The Yodita Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The Yodita Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The Yodita Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The Yodita Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$YODITA_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$YODITA_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$YODITA_ROOT"/prebuilts/tools-lineage/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$YODITA_ROOT"/prebuilts/tools-lineage/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$YODITA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$YODITA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$YODITA_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$YODITA_ROOT"/vendor/yodita/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$YODITA_ROOT"/"$OUTDIR"/radio
